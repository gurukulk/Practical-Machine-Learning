...
99.77% is a good number for accuracy which totally validates the idea / hypothesis we made to eliminate most variables and use only 9 relatively independent covariates.

###Estimation of the out-of-sample error rate

The train2 test set was removed and left untouched during variable selection, training and optimizing of the Random Forest algorithm. Therefore this testing subset gives an unbiased estimate of the Random Forest algorithm’s prediction accuracy (99.77% as calculated above). The Random Forest’s out-of-sample error rate is derived by the formula 100% - Accuracy, or can be calculated directly by the following lines of code:
...
The out-of-sample error rate is 0.19%.
...
We create one file with all the results, presented in two columns (named problem_id and classe) and 20 rows of data:
...
The accuracy obtained (accuracy = 99.77%, and out-of-sample error = 0.19%) is obviously highly suspicious as it is never the case that machine learning algorithms are that accurate, and a mere 85% if often a good accuracy result.
